...
import { textureAsync } from './ExpoPixi';
...
async function spineAsync({ json, atlas, assetProvider }) {
  ...
  const atlasUrl = await AssetUtils.uriAsync(atlas);
  const _atlas = await FileSystem.readAsStringAsync(atlasUrl);
  ...
  const customAssetProvider = async (line, callback) => {
    ...
    if (asset) {
      if (asset instanceof PIXI.BaseTexture) {
        ...
        const texture = await textureAsync(asset);
        if (texture) {
